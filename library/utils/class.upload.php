...
class upload {
     ...
     */
    function process($server_path = null,$thumb = 0) {
        ...
        if ($this->processed) {
			...
			if($thumb==1){
				...
				$this->file_dst_pathname = $this->file_dst_path .$this->file_dst_name_body .'.thumb'. (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
			} else {
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    ...
                    while (@file_exists($this->file_dst_pathname)) {
						...
						if($thumb==1){
							$this->file_dst_name_body   = $body . '_' . $cpt.'.thumb';
						}
                        $this->file_dst_name = $this->file_dst_name_body . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
                        $cpt++;
                        $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
